    ...
    <div className="absolute inset-0 z-[2] opacity-40 mix-blend-screen pointer-events-none flex items-end justify-between px-2 sm:px-10 pb-10 overflow-hidden">
      {bars.map((bar) => (
        <motion.div
          ...
          transition={{duration: bar.duration1, repeat: Infinity, delay: bar.delay1}}
          style={{height: '70%'}}
        >
          <motion.div className={`w-[1px] absolute top-1/4 bottom-1/4 ${bar.isGreen ? 'bg-emerald-500' : 'bg-red-500'}`} />
          <motion.div
            initial={{height: `${bar.baseHeight}%`}}
            animate={{height: [`${bar.baseHeight}%`, `${bar.baseHeight + bar.heightDelta}%`, `${bar.baseHeight}%`]}}
            transition={{duration: bar.duration2, repeat: Infinity, ease: 'easeInOut'}}
